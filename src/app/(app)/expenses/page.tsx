  ...
  async function load() {
    ...
    try {
      const [txData, accData, catData] = await Promise.all([
        listTransactions({ direction: "out" }),
        listAccounts(),
        listCategories("out"),
      ]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowForm(!showForm)} className="rounded-full bg-rose-500 px-4 py-2 text-sm font-semibold text-white">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="rounded-xl border border-slate-800 bg-slate-900/80 p-4 space-y-3">
          ...
          <button type="submit" disabled={saving} className="primary">{saving ? "Guardando..." : "Guardar gasto"}</button>
        </form>
      )}

      <div className="rounded-xl border border-slate-800 bg-slate-900/80 p-4">
        <p className="text-xs text-slate-500">Total gastos</p>
        <p className="text-2xl font-bold text-rose-400">{formatMoney(total)}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {transactions.length === 0 ? (
            <p className="text-sm text-slate-500 text-center py-8">No hay gastos registrados</p>
          ) : (
            transactions.map((tx: any) => (
              <div key={tx.id} className="flex items-center justify-between rounded-xl border border-slate-800 bg-slate-900/50 p-4">
                ...
                <div className="flex items-center gap-3">
                  <span className="text-sm font-semibold text-rose-400">{formatMoney(tx.amount, tx.currency)}</span>
